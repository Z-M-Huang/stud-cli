/**
 * /network-policy deny subcommand — add a host to the denylist after confirmation.
 *
...
async function confirmDeny(host: string, hostApi: HostAPI): Promise<boolean> {
  const result = await hostApi.interaction.raise({
    kind: "confirm",
    prompt: `Add '${host}' to the network policy denylist?`,
  });
...
/**
 * Execute the `deny <host>` subcommand.
 *
 * Validates the hostname, optionally prompts for confirmation, adds the host
 * to the denylist, and emits a `NetworkPolicyChange` audit record.
 *
 ...
 */
export async function executeDeny(
  host: string,
  ...
  if (requireConfirm) {
    const confirmed = await confirmDeny(host, hostApi);
    if (!confirmed) {
      throw new Cancellation("user declined to add to denylist", undefined, {
        code: "TurnCancelled",
        host,
      });
    }
  ...
  await hostApi.audit.write({
    ...
    message: `/network-policy deny added '${host}' to the denylist`,
    context: { action: "deny", host },
  });

  return {
    rendered: `'${host}' added to the network policy denylist.`,
